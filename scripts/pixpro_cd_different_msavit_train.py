...
from organdetr.pixpro_cd_different_msavit_trainer import PixProTrainer
from organdetr.data.dataloader import get_loader
from organdetr.utils.io import get_config, write_json, get_meta_data
from organdetr.models.pixpro_different_msavit_organdetrnet import PixProOrgandetrNet
...
def train(config, args):
    ...
    source_train_loader = get_loader(config, 'train', batch_size=config['source_batch_size'], root=config['source_dataset_dir'])
    target_labeled_train_loader = get_loader(config, 'labeled_train', batch_size=config['target_labeled_batch_size'], root=config['target_dataset_dir'])
    target_unlabeled_train_loader = get_loader(config, 'unlabeled_train', batch_size=config['target_unlabeled_batch_size'], root=config['target_dataset_dir'])
    ...
    config['pixpro']['num_instances'] = len(target_unlabeled_train_loader)
    ...
    trainer = PixProTrainer(
        source_train_loader, target_labeled_train_loader, target_unlabeled_train_loader, val_loader, model, criterion, optim, scheduler, device, config, 
