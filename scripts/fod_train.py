...
from organdetr.fod_trainer import Trainer
...
def train(config, args):
    ...
    target_train_loader = get_loader(config, 'train', is_target=True)

    if config['overfit']:
        val_loader = get_loader(config, 'train', is_target=True)
    else:
        val_loader = get_loader(config, 'val', is_target=True)
    ...
    trainer = Trainer(
        ...
        path_to_run, epoch, metric_start_val, dense_hybrid_criterion, target_train_loader,
    )
    trainer.run()
