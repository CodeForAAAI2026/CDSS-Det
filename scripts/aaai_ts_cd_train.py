...
from organdetr.aaai_ts_cd_trainer import TSTrainer
...
def train(config, args):
    ...
    pseudo_split = config.get('pseudo_split', 'train')
    ...
    pseudo_train_loader = get_loader(config, pseudo_split, batch_size=config['pseudo_batch_size'], root=config['target_dataset_dir'], replay_samples=None, pixpro=False)
    ...
    if config.get('resume', False) or args.resume:
        ...
        pseudo_cls_coef = checkpoint.get('pseudo_cls_coef', config['loss_coefs']['pseudo'])
    else:
        pseudo_cls_coef = None
        epoch = 0
        metric_start_val = 0
        if config['use_pre_trained_model']:
            # Load pre-trained model
            old_model_path = config['old_model_path']
            print(f'[+] Loading checkpoint from {old_model_path} ...')
            ...
            for key in model_state_dict:
                if key in old_model_state_dict and model_state_dict[key].shape == old_model_state_dict[key].shape:
                    new_model_state_dict[key] = old_model_state_dict[key]  # Load pre-trained weights
                else:
                    new_model_state_dict[key] = model_state_dict[key]  # Keep new model's weights for mismatched layers

                    if key in old_model_state_dict:
                        print(f"[!] Skipping layer '{key}' due to shape mismatch: "
                            f"{old_model_state_dict[key].shape} -> {model_state_dict[key].shape}")

            # Load updated weights
            model.load_state_dict(new_model_state_dict, strict=False)
            teacher_model.load_state_dict(new_model_state_dict, strict=False)

            print("[+] Successfully loaded pre-trained model (excluding mismatched layers).")
    ...
    trainer = TSTrainer(
        ...
        path_to_run, epoch, metric_start_val, dense_hybrid_criterion, pseudo_cls_coef
    )
    trainer.run()
