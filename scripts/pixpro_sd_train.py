...
from organdetr.pixpro_sd_trainer import PixProTrainer
...
def train(config, args):
    ...
    labeled_train_loader = get_loader(config, config['labeled_train_set'], batch_size=config['batch_size'])
    pixpro_train_loader = get_loader(config, config['unlabeled_train_set'], batch_size=config['pixpro']['batch_size'], root=None, replay_samples=None, weak_aug=False, pixpro=True)
    pseudo_train_loader = get_loader(config, config['unlabeled_train_set'], batch_size=config['pseudo_batch_size'], root=None, replay_samples=None, weak_aug=True, pixpro=False)

    if config['overfit']:
        val_loader = get_loader(config, 'train')
    else:
        val_loader = get_loader(config, 'val', batch_size=4)
