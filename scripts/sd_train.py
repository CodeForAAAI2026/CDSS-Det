...
def train(config, args):
    ...
    train_loader = get_loader(config, config['split'])
    ...
    if config.get('resume', False) or args.resume:
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=config['lr_drop'])
        scheduler.last_epoch = epoch  # Continue from last saved epoch
    ...
    if config.get('resume', False) or args.resume:
        ...
        # Read new lr_drop from config file
        new_lr_drop = config['lr_drop']
        initial_lr = float(config['lr'])  # Your initial learning rate
        decay_factor = float(config['lr_gamma'])  # Default decay factor for StepLR

        # Compute how many times the LR should have decayed under the new rule
        num_decays = epoch // new_lr_drop
        new_lr = initial_lr * (decay_factor ** num_decays)  # Compute correct LR

        # Apply manually computed learning rate to optimizer
        for param_group in optim.param_groups:
            param_group['lr'] = new_lr

        # Reinitialize the scheduler with the new step size
        scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=new_lr_drop)

        print(f"Resumed from epoch {epoch}, new learning rate: {optim.param_groups[0]['lr']}")
