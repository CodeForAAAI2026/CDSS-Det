...
import io
from organdetr.utils.MMD import get_ins_feature
from organdetr.utils.gmm_index import get_index_via_gmm
from organdetr.data.dataloader import CombinedDataLoader
...
class Trainer:
    ...
    def __init__(
        ...
        self._target_train_loader = target_train_loader
        ...
        # target dataset
        self._target_data = torch.tensor([]).to(self._device)
        self._target_det_targets = []
        self._target_seg_targets = torch.tensor([]).to(self._device)
        target_list = list(self._target_train_loader)
        for data, _, boxes, seg_targets in target_list:
            data = data.to(device=self._device)
            seg_targets = seg_targets.to(device=self._device)
            self._target_data = torch.cat((self._target_data, data))
            self._target_seg_targets = torch.cat((self._target_seg_targets, seg_targets))
            for item in boxes:
                target = {
                ...
                self._target_det_targets.append(target)
            
        print(f"=======================len(self._target_det_targets)={len(self._target_det_targets)}")
        print(f"target_det_targets[0]['labels']={self._target_det_targets[0]['labels']}")
        print(f"target_det_targets[1]['labels']={self._target_det_targets[1]['labels']}")
        #print(f"target_det_targets[7]['labels']={self._target_det_targets[7]['labels']}")
    ...
    def _train_one_epoch(self, num_epoch):
        ...
        """
        combined_loader = CombinedDataLoader(self._train_loader, self._target_train_loader,
                                             self._config['batch_size'], 1)
        print(f"===============len(combined_loader)={len(combined_loader)}")
        ...
        for data, _, bboxes, seg_targets in progress_bar:
            ...
            #print(f"==========================self._target_data.shape={self._target_data.shape}")
            random_idx = np.random.choice(self._target_data.shape[0])
            #print(f"==========================self._target_data[random_idx].shape={self._target_data[random_idx].shape}")
            #print(f"=========random_idx={random_idx}")
            data = torch.cat((data, self._target_data[random_idx].unsqueeze(0)))
            seg_targets = torch.cat((seg_targets, self._target_seg_targets[random_idx].unsqueeze(0)))
            ...
            det_targets.append(self._target_det_targets[random_idx])

            # print(f"=============len(det_targets)={len(det_targets)}, det_targets[8]['labels']={det_targets[8]['labels']}")
            # Make prediction
            with autocast(): 
                out, contrast_losses, dn_meta, features = self._model(data, det_targets, num_epoch=num_epoch)
                if self._config['gmm']:
                    _, _, _, target_features = self._model(self._target_data, self._target_det_targets, num_epoch=num_epoch)

                    instance_features = get_ins_feature(features[0], det_targets, self._device)
                    target_instance_features = get_ins_feature(target_features[0], self._target_det_targets, self._device)
                    #print(f"=========================features[0].shape={features[0].shape}")
                    #print(f"=========================instance_features.shape={instance_features.shape}")
                    k = instance_features.shape[0] * 0.2
                    tar_dissim_idx, tar_sim_idx = get_index_via_gmm(instance_features, target_instance_features, int(k))
                    print(f"=========instance_features.shape={instance_features.shape}, k={k}, tar_sim_idx={tar_sim_idx}")
                    #print(f"=========================tar_dissim_idx={tar_dissim_idx}")
                    #print(f"=========================tar_sim_idx={tar_sim_idx}")

                    tar_sim = instance_features[tar_sim_idx, :].detach()
                    tar_dissim = instance_features[tar_dissim_idx, :]
                    target_instance_features = torch.cat((target_instance_features, tar_sim))

                    loss_dict, pos_indices = self._criterion(out, det_targets, seg_targets, tar_dissim, target_instance_features, dn_meta, num_epoch=num_epoch)
                else:
                    loss_dict, pos_indices = self._criterion(out, det_targets, seg_targets, None, None, dn_meta, num_epoch=num_epoch)
                ...
                for loss_key, loss_val in loss_dict.items():
                    # print(f"===========================loss_key={loss_key}, loss_val={loss_val}")
            ...
            loss_asy_agg += loss_dict['asy'].item()
        ...
        loss_asy = loss_asy_agg / len(self._train_loader)
    ...
    def _validate(self, num_epoch):
        ...
        print(f"===============================epoch={num_epoch}, total_loss={loss}, mAP_coco={metric_scores['mAP_coco']}") 
