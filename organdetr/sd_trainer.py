...
class Trainer:
    ...
    def _train_one_epoch(self, num_epoch):
        ...
        print(f"==========================len(self._train_loader)={len(self._train_loader)}")
        for data, weak_data, seg_mask, bboxes, weak_bboxes, seg_targets, weak_seg_targets, path in progress_bar:
            # Put data to gpu
            ...
            # Make prediction
            with autocast(): 
                out, contrast_losses, dn_meta, _ = self._model(data, det_targets, num_epoch=num_epoch)
                #print(f"==========================out['pred_logits'].shape={out['pred_logits'].shape}")
                #print(f"==========================len(det_targets)={len(det_targets)}, det_targets[0]['labels'].shape={det_targets[0]['labels'].shape}, det_targets[0]['labels']={det_targets[0]['labels']}")
