...
from organdetr.utils.io import write_ply
...
def save_pred_visualization_nifti(
    pred_boxes, pred_classes, gt_boxes, gt_classes, seg_mask, path, classes, idx, input_image=None, config=None
):
    """Saves predictions, gt, and, data as .nii.gz/.json files for visualization purposes with 3D Slicer.
    ...
    Args:
        ...
        input_image: The input CT image which will be exported.
    """
    ...
    path_to_instance.mkdir(exist_ok=True)
     ...
     # init nifti file
    affine = np.eye(4)  # You can adjust this as needed
    seg_mask_nifti_image = nib.Nifti1Image(seg_mask, affine)
    
    if input_image is not None:
        input_data = np.squeeze(input_image)
        assert len(input_data.shape) == 3
        image_nifti_image = nib.Nifti1Image(input_data, affine)
        nib.save(image_nifti_image, os.path.join(path_to_instance, 'img.nii.gz'))
    nib.save(seg_mask_nifti_image, os.path.join(path_to_instance, 'seg_mask.nii.gz'))

    # Generate point cloud of each class and gt boxes
    export_markup_json(seg_mask, gt_boxes, gt_classes, classes, True, path_to_instance, config)
    # Generate pred bboxes
    export_markup_json(seg_mask, pred_boxes, pred_classes, classes, False, path_to_instance, config)
    
def export_markup_json(segmentation_map, bounding_boxes, classes, classes_dict, gt, path, config):
    """Exports .mrk.json that can be imported by 3D Slicer"""
    edges_map = np.zeros_like(segmentation_map)
    for i, bbox in enumerate(bounding_boxes):
        with open('config/template_bbox.mrk.json', 'r') as f:
            json_data = json.load(f)
        
        org_bbox = rescale_bbox(bbox, np.array(segmentation_map.shape) - 1)
        #org_bbox = org_bbox.astype(np.int)
        center_x, center_y, center_z, width, height, depth = org_bbox
        #start = [center_x - width//2, center_y - height//2, center_z - depth//2]
        #end = [center_x + width//2, center_y + height//2, center_z + depth//2]

        if str(classes[i]) in config['labels_small']:
            size = 's'
        elif str(classes[i]) in config['labels_mid']:
            size = 'm'
        elif str(classes[i]) in config['labels_large']:
            size = 'l'

        
        # write markup json
        json_data['markups'][0]['center'] = [float(center_x), float(center_y), float(center_z)]
        json_data['markups'][0]['size'] = [float(width), float(height), float(depth)]
        json_data['markups'][0]['controlPoints'][0]['position'] = [float(center_x), float(center_y), float(center_z)]
        json_data['markups'][0]['display']['propertiesLabelVisibility'] = False
        class_str = classes_dict[str(classes[i])].replace('_','-')
        if gt:
            json_data['markups'][0]['display']['selectedColor'] = [0.0, 1.0, 0.0]  # green for GT
            json_dest = f'{size}-{str(classes[i]).zfill(2)}-{class_str}_gt_bbox.json'
        else:
            json_data['markups'][0]['display']['selectedColor'] = [1.0, 0.0, 0.0]  # red for pred
            json_dest = f'{size}-{str(classes[i]).zfill(2)}-{class_str}_pred_bbox.json'
        with open(os.path.join(path, json_dest), 'w') as json_file:
...
def save_pred_visualization(
    ...
    for bbox, class_ in zip(gt_boxes, gt_classes):
        ...
        class_str = classes[str(class_)].replace('_','-')
    ...
    for bbox, class_ in zip(pred_boxes, pred_classes):
        class_str = classes[str(class_)].replace('_','-')
        bbox = rescale_bbox(bbox, seg_mask.shape)
        print("pdbox: ",class_str)
        bbox = np.concatenate((bbox, np.array([0])), axis=0)
        path_bbox = str(path_to_instance) + f'/{class_str}_bbox_pred.ply'
