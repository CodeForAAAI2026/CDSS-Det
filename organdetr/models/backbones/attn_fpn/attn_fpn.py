"""Module containing code of the organdetr projects backbone."""
...
from organdetr.models.backbones.attn_fpn.decoder_blocks import DecoderDefAttnBlock
from organdetr.models.backbones.attn_fpn.encoder_blocks import (
    EncoderCnnBlock,
    EncoderSwinBlock,
    PatchMerging,
    ConvPatchMerging
    ...
    def __init__(self, config, debug):
        ...
        for stage_id in range(num_stages):
            ...
            if config['use_encoder_attn'] and stage_id > 1: # Initial patch embedding done with convs
                swin_enc_block = EncoderSwinBlock(
                    dim=in_channels*2, # after the first CNN block, we need to double the channels
