...
# TODO
# EncoderCnnBlock and Decoder are adopted from Organdetr ...

'''
class EncoderCnnBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding=1, bias=False, affine=True, eps=1e-05):
        super().__init__()
    ...
    def forward(self, x):
        x = self._block[0](x)  # Conv3d
        print(f"After conv1: {x.abs().max().item()}")

        x = self._block[1](x)  # InstanceNorm3d
        print(f"After InstanceNorm1: {x.abs().max().item()}")

        x = self._block[2](x)  # ReLU
        print(f"After ReLU1: {x.abs().max().item()}")

        x = self._block[3](x)  # Conv3d
        print(f"After conv2: {x.abs().max().item()}")

        x = self._block[4](x)  # InstanceNorm3d
        print(f"After InstanceNorm2: {x.abs().max().item()}")

        x = self._block[5](x)  # ReLU
        print(f"After ReLU2: {x.abs().max().item()}")
