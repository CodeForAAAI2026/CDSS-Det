"""Main model of the organdetr project."""
...
from organdetr.models.build import build_backbone, build_neck, build_pos_enc
from organdetr.models.necks.def_detr_transformer import inverse_sigmoid
from organdetr.models.necks.cdn import dn_post_process, prepare_for_cdn, prepare_for_dn
from organdetr.models.matcher import HungarianMatcher
from organdetr.utils.io import load_json
...
class PixProOrgandetrNet(nn.Module):
    def __init__(self, config):
        ...
        # load data info
        data_path = os.environ.get('ORGANDETR_DATA')
        ...
        self._use_pixpro = False
        self._pixpro_same_encoder = False
        # Get pixpro
        if config['pixpro']['use_pixpro']:
            self._use_pixpro = True
            encoder_k = build_backbone(config['backbone'])
            if config['pixpro']['same_encoder']:
                self._pixpro_same_encoder = True
                self._pixpro = PixPro(self._backbone, encoder_k, config)
            else:
                encoder_q = build_backbone(config['backbone'])
                self._pixpro = PixPro(encoder_q, encoder_k, config)
        ...
        # Get discriminator
        self._discriminator = FCDiscriminator_img_3D(self.hidden_dim)
        ...
        print(f"==============out_channals={out_channels}")
    ...
    def get_domain_loss(self, feature, domain='source'):
        ...
        target = torch.full_like(logits, fill_value=label, device=self.device)

        '''
        epsilon = 1e-8
        # Convert logits to probabilities
        probabilities = torch.sigmoid(logits)
        # Apply clamping to avoid log(0)
        clamped_probabilities = torch.clamp(probabilities, epsilon, 1 - epsilon)
        # Calculate binary cross entropy
        # loss = F.binary_cross_entropy(clamped_probabilities, target)
        '''
        loss = self.binary_cross_entropy_with_logits_eps(logits, target)
        return loss


    def forward(self, x, targets=None, num_epoch: int=-1, data_q=None, data_k=None, crop_coords_q=None, crop_coords_k=None, domain_loss=False): # in trainer, None when !self.training
        # targets: list of dict{'boxes': , 'labels':}

        #print(f"============================x.shape={x.shape}")
        out_backbone = self._backbone(x)
        #print(f"============================out_backbone.keys()={out_backbone.keys()}")
        #print(f"============================out_backbone['P4'].shape={out_backbone['P4'].shape}")
        ...
        if self.hybrid and self.training:
            ...
            #print(f"======================model.forward, pred_logits.shape={pred_logits.shape}, self.num_queries={self.num_queries}")
            #print(f"======================model.forward, pred_logits[0][0][0][:]={pred_logits[0][0][0][:]}")
            out = {
        ...
        if self.training:
            ...
            if domain_loss:
                # P4?
                # print(f"=========det_srcs[0].shape={det_srcs[0].shape}")
                # Replay: target + source
                '''
                target_domain_loss = torch.tensor(0.0, device=self.device)
                source_domain_loss = torch.tensor(0.0, device=self.device)
                for feat in det_srcs:
                    target_domain_loss += self.get_domain_loss(feat[0], domain='target')
                    source_domain_loss += self.get_domain_loss(feat[1], domain='source')

                other_losses['source_domain_loss'] = source_domain_loss
                other_losses['target_domain_loss'] = target_domain_loss
                '''
                # Only use P4, P3 and P2 are easier to get NaN
                other_losses['source_domain_loss'] = self.get_domain_loss(det_srcs[0][1], domain='source')
                other_losses['target_domain_loss'] = self.get_domain_loss(det_srcs[0][0], domain='target')
            else:
                other_losses['source_domain_loss'] = torch.tensor(0.0, device=self.device) 
                other_losses['target_domain_loss'] = torch.tensor(0.0, device=self.device) 

            # only use pixpro for training
            if self._use_pixpro and data_q is not None and data_k is not None and crop_coords_q is not None and crop_coords_k is not None:
                # no segmentation label
                self._pixpro.encoder._decoder.msa_seg = False
                self._pixpro.encoder_k._decoder.msa_seg = False
                pixpro_loss, pixpro_losses = self._pixpro(data_q, data_k, crop_coords_q, crop_coords_k)
                # se it back for supervised branch
                self._pixpro.encoder._decoder.msa_seg = True
                self._pixpro.encoder_k._decoder.msa_seg = True
                other_losses['pixpro_loss'] = pixpro_loss
                other_losses['pixpro_losses'] = pixpro_losses
                # return out, con_losses, dn_meta, det_srcs, pixpro_loss, pixpro_losses
                return out, con_losses, dn_meta, det_srcs, other_losses
            else:
                other_losses['pixpro_loss'] = torch.tensor(0.0, device=self.device)
                other_losses['pixpro_losses'] = [torch.tensor(0.0, device=self.device)]
                # return out, con_losses, dn_meta, det_srcs, torch.tensor(0), [torch.tensor(0)]
                return out, con_losses, dn_meta, det_srcs, other_losses
