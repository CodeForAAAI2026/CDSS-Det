...
class TSOrgandetrNet(nn.Module):
    ...
    def forward(self, x, targets=None, num_epoch: int=-1): # in trainer, None when !self.training
        ...
        for lvl in range(hs.shape[0]):
                ...
                with torch.no_grad():
                    ...
                    for i, (name, module) in enumerate(self.named_children()):
                        if hasattr(module, 'output'):
                            if torch.isnan(module.output).any():
                                print(f"NaN detected after module: {name}")

                if torch.isnan(hs[lvl]).any():
                    print("NaN detected in hs[lvl]")

                tmp = self._bbox_reg_head(hs[lvl])

                if torch.isnan(tmp).any():
                    print("NaN detected in tmp")


            if torch.isnan(reference).any():
                print("NaN detected in reference")
            ...
            if torch.isnan(outputs_coord).any():
                print("NaN detected in outputs_coord")
        ...
        if self.training:
            ...
            # print(f"==================before return, domain_loss={domain_loss}")
            # return out, con_losses, dn_meta, det_srcs, other_losses
            '''
            return out, con_losses, dn_meta, det_srcs
