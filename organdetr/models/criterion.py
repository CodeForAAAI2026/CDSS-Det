"""Module containing the loss functions of the organdetr project."""
import copy
import torch
...
from organdetr.utils.bboxes import generalized_bbox_iou_3d, box_cxcyczwhd_to_xyzxyz
...
class OrgandetrCriterion(nn.Module):
    """ This class computes the loss for OrgandetrNet.
    ...
    def loss_class(self, outputs, targets, indices, num_boxes=None):
        ...
        # print(f"=========len(indices)={len(indices)}, indices={indices}, idx={idx}")
        ...
        # print(f"============src_logits.shape={src_logits.shape}, target_classes.shape={target_classes.shape}")
    ...
    def loss_segmentation(self, outputs, targets):
        ...
        if self._seg_msa:
            ...
            for output, output_neck in zip(outputs['pred_seg'], outputs['neck_enc_seg']):
                ...
                # print(f"=========================targets.shape={targets.shape}, output.size()={output.size()}, out_size={out_size}")
                ...
                # print(f"=========================output.shape={output.shape}, output_neck.shape={output_neck.shape}, target.shape={target.shape}")
        ...
        fix_sigma: 表示是否使用固定的标准差
            return: (sample_size_1 + sample_size_2) * (sample_size_1 + sample_size_2)的
                            ...
                            [   K_ss K_st
                                K_ts K_tt ]
        """
        # print(f"======================kernel, source.shape={source.shape}")
    ...
    def get_mmd_loss(self, source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
        n = int(source.size()[0])
        m = int(target.size()[0])
    ...
    def forward(self, outputs, targets, seg_targets, feature_source = None, feature_target = None, dn_meta=None, num_epoch:int=-1):
        ...
        if feature_source is not None and feature_target is not None:
            loss_dict = self.get_mmd_loss(feature_source, feature_target)
            losses.update(loss_dict)
        else:
            losses['asy'] = torch.tensor(0).float().cuda()
