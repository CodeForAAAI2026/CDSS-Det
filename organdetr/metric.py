"""
Module containing the metric of the organdetr project.
...
class Metric:
    ...
    def compute_ar(self, dataset_statistics):
        ...
        for max_det_idx, max_det in enumerate(self.max_detections):  # mAR
            if self.iou_range_coco:
                key = f"mAR_coco"
                results[key] = self.select_ar(dataset_statistics, max_det_idx=max_det_idx, iou_idx=self.iou_range_coco_idx)
                
                if self.classes_subsets:    #mARs, mARm, mARl coco
                    for cls_idx, cls_str in enumerate(self.classes):  # per class results
                        for key_, dict_ in self.classes_subsets.items():
                            key = (f"mAR_coco_{key_}")
                            cls_idx = [int(cls_id) -1 for cls_id in list(dict_.keys())]
                            results[key] = self.select_ar(dataset_statistics,
                                                        cls_idx=cls_idx, max_det_idx=max_det_idx, iou_idx=self.iou_range_coco_idx)
                if self.per_class:
                    for cls_idx, cls_str in enumerate(self.classes):  # per class results
                        key = (f"mAR_coco_{cls_str}")
                        results[key] = self.select_ar(dataset_statistics,
                                                    cls_idx=cls_idx, max_det_idx=max_det_idx, iou_idx=self.iou_range_coco_idx)
            if self.iou_range_nndet:
                key = f"mAR_nndet"
                results[key] = self.select_ar(dataset_statistics, max_det_idx=max_det_idx, iou_idx=self.iou_range_nndet_idx)
                
                if self.classes_subsets:    #mARs, mARm, mARl nndet
                    for cls_idx, cls_str in enumerate(self.classes):  # per class results
                        for key_, dict_ in self.classes_subsets.items():
                            key = (f"mAR_nndet_{key_}")
                            cls_idx = [int(cls_id) -1 for cls_id in list(dict_.keys())]
                            results[key] = self.select_ar(dataset_statistics,
                                                        cls_idx=cls_idx, max_det_idx=max_det_idx, iou_idx=self.iou_range_nndet_idx)
                if self.per_class:
                    for cls_idx, cls_str in enumerate(self.classes):  # per class results
                        key = (f"mAR_nndet_{cls_str}")
                        results[key] = self.select_ar(dataset_statistics,
                                                    cls_idx=cls_idx, max_det_idx=max_det_idx, iou_idx=self.iou_range_nndet_idx)


        for idx in self.iou_list_idx:   # AR@IoU
            key = f"AR_IoU_{self.iou_thresholds[idx]:.2f}"
            results[key] = self.select_ar(dataset_statistics, iou_idx=idx, max_det_idx=-1)

            if self.per_class:
                for cls_idx, cls_str in enumerate(self.classes):  # per class results
                    key = (f"AR_IoU_{self.iou_thresholds[idx]:.2f}_"
                           f"{cls_str}")
                    results[key] = self.select_ar(dataset_statistics, 
