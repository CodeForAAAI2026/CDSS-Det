...
class PixProTrainer:
    ...
    def __init__(
        self, source_train_loader, target_labeled_train_loader, target_unlabeled_train_loader, val_loader, model, criterion, optimizer, scheduler,
        device, config, path_to_run, epoch, metric_start_val, dense_hybrid_criterion
    ):
        self._source_train_loader = source_train_loader
        self._target_labeled_train_loader = target_labeled_train_loader 
        self._target_unlabeled_train_loader = target_unlabeled_train_loader 
    ...
    def _select_topk_unlabeled(self):
        self._model.eval()
        loader = get_loader(self._config, 'unlabeled_train', batch_size=1, root=self._config['target_dataset_dir'])
        progress_bar = tqdm(loader)
        losses = {}
        for idx, (data_q, data_k, crop_coords_q, crop_coords_k, path) in enumerate(progress_bar):
            # Put data to gpu
            ...
            pixpro_loss, _ = self._model._pixpro(data_q, data_k, crop_coords_q, crop_coords_k)
            losses[path[0]] = pixpro_loss.item()

        sorted_losses = sorted(losses.items(), key=lambda item: item[1], reverse=True)
        top_k = sorted_losses[:self._config['top_k_unlabeled']]

        remaining = sorted_losses[self._config['top_k_unlabeled']:]
        random_samples = random.sample(remaining, self._config['random_unlabeled'])

        final_data = top_k + random_samples
        final_paths = [item[0] for item in final_data]

        set1 = set(self._last_final_paths)
        set2 = set(final_paths)
        diff = set1.symmetric_difference(set2)
        self._top_k_diff = len(diff)

        self._last_final_paths = final_paths
        self._target_unlabeled_train_loader = get_loader(self._config, 'unlabeled_train', batch_size=self._config['target_unlabeled_batch_size'], root=self._config['target_dataset_dir'], data_list=final_paths)
    ...
    def _train_one_epoch(self, num_epoch):
        ...
        print(f"==============len(self._source_train_loader)={len(self._source_train_loader)}, len(self._target_labeled_train_loader)={len(self._target_labeled_train_loader)}, len(self._target_unlabeled_train_loader)={len(self._target_unlabeled_train_loader)}, len(self._val_loader)={len(self._val_loader)}")
        # progress_bar = tqdm(self._labeled_train_loader)
        # for idx, (data, _, bboxes, seg_targets) in enumerate(progress_bar):
        progress_bar = tqdm(zip(self._source_train_loader, self._target_labeled_train_loader, self._target_unlabeled_train_loader))
        for idx, ((source_data, _, source_bboxes, source_seg_targets, _), (data, _, bboxes, seg_targets, _), (data_q, data_k, crop_coords_q, crop_coords_k, _)) in enumerate(progress_bar):
            # Put data to gpu
            ...
            # data = torch.cat((data, target_data), dim=0)
            # seg_targets = torch.cat((seg_targets, target_seg_targets), dim=0)
            ...
            with autocast(): 
                # source
                source_out, source_contrast_losses, source_dn_meta, _, = self._model(source_data, source_det_targets, num_epoch)
                ...
                source_loss_dict['pixpro'] = torch.tensor(0)
                del source_loss_dict['hd95']

                #print(f"=============data.shape={data.shape}, data_q.shape={data_q.shape}, data_k.shape={data_k.shape}, crop_coords_q.shape={crop_coords_q.shape}, crop_coords_k.shape={crop_coords_k.shape}")
                out, contrast_losses, dn_meta, _, pixpro_loss, _ = self._model(data, det_targets, num_epoch, data_q, data_k, crop_coords_q, crop_coords_k)
                out, contrast_losses, dn_meta, _, = self._model(data, det_targets, num_epoch)
                loss_dict, pos_indices = self._criterion(out, det_targets, seg_targets, dn_meta, num_epoch=num_epoch)
                loss_dict['pixpro'] = pixpro_loss
                ...
                for loss_key, loss_val in source_loss_dict.items():
                    source_loss_abs += loss_val * self._config['loss_coefs'][loss_key.split('_')[0]]
                for loss_key, loss_val in source_contrast_losses.items():
                    source_loss_abs += loss_val # already multiplied coefficient in organdetrnet.py
                    # source_loss_contrast_agg += loss_val 
            ...
            if self._config['gradient_accu']:
                ...
                if ((idx + 1) % accum_iter == 0) or (idx + 1 == len(self._target_labeled_train_loader)):
        ...
        source_loss = source_loss_agg / len(self._target_labeled_train_loader)
        loss = loss_agg / len(self._target_labeled_train_loader)
        #print(f'total train loss for epoch {num_epoch}: '+str(loss))
        loss_bbox = loss_bbox_agg / len(self._target_labeled_train_loader)
        loss_giou = loss_giou_agg / len(self._target_labeled_train_loader)
        loss_cls = loss_cls_agg / len(self._target_labeled_train_loader)
        loss_pixpro = loss_pixpro_agg / len(self._target_labeled_train_loader)
        loss_seg_ce = loss_seg_ce_agg / len(self._target_labeled_train_loader)
        loss_seg_dice = loss_seg_dice_agg / len(self._target_labeled_train_loader)
        if self._hybrid: # hybrid matching
            loss_bbox_one2many = loss_bbox_one2many_agg / len(self._target_labeled_train_loader)
            loss_giou_one2many = loss_giou_one2many_agg / len(self._target_labeled_train_loader)
            loss_cls_one2many = loss_cls_one2many_agg / len(self._target_labeled_train_loader)
            loss_seg_ce_one2many = loss_seg_ce_one2many_agg / len(self._target_labeled_train_loader)
            loss_seg_dice_one2many = loss_seg_dice_one2many_agg / len(self._target_labeled_train_loader)
        
        if self._criterion._seg_proxy:  # log Hausdorff
            seg_hd95 = hd95_agg / len(self._target_labeled_train_loader)
        else:
            seg_hd95 = 0

        loss_contrast = loss_contrast_agg / len(self._target_labeled_train_loader)
        
        if len(loss_dn_agg) != 0:
            for key in loss_dn_agg:
                value = loss_dn_agg[key] / len(self._target_labeled_train_loader)
                self._writer.add_scalar("dn/"+key, value, num_epoch)
        if len(loss_aux_agg) != 0:
            for key in loss_aux_agg:
                value = loss_aux_agg[key] / len(self._target_labeled_train_loader)
                self._writer.add_scalar("train_aux/"+key, value, num_epoch)
        if len(loss_aux_agg_dn) != 0:
            for key in loss_aux_agg_dn:
                value = loss_aux_agg_dn[key] / len(self._target_labeled_train_loader)
                self._writer.add_scalar("dn/"+key, value, num_epoch)
        if loss_enc_bbox_agg or loss_enc_giou_agg or loss_enc_cls_agg:
            self._writer.add_scalar("train_enc/bbox_enc", loss_enc_bbox_agg/len(self._target_labeled_train_loader), num_epoch)
            self._writer.add_scalar("train_enc/giou_enc", loss_enc_giou_agg/len(self._target_labeled_train_loader), num_epoch)
            self._writer.add_scalar("train_enc/cls_enc", loss_enc_cls_agg/len(self._target_labeled_train_loader), num_epoch)
        ...
        if self._hybrid: # log many2one just if hybrid matching is activated
            ...
            print(f"==================epoch={num_epoch}, source_total_loss={source_loss}, total_loss={loss}, pixpro_loss={loss_pixpro}, top_k_diff={self._top_k_diff}")
