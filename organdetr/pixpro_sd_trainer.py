...
class PixProTrainer:
    ...
    def __init__(
        ...
        # self._unlabeled_train_loader = unlabeled_train_loader
    ...
    def _train_one_epoch(self, num_epoch):
        ...
        print(f"==============len(self._labeled_train_loader)={len(self._labeled_train_loader)}, len(self._pixpro_train_loader)={len(self._pixpro_train_loader)}, len(self._pseudo_train_loader)={len(self._pseudo_train_loader)}")
        ...
        for idx, ((data, _, bboxes, seg_targets, _), (data_q, data_k, crop_coords_q, crop_coords_k, _), (pseudo_data, _, _, _, _)) in enumerate(progress_bar):
            # Put data to gpu
            ...
            if self._config['pixpro']['use_pixpro']:
                ...
                # Here these four values might not be from unlabeled dataset and not be meaningful, just ignore them. 
                data_q, data_k, crop_coords_q, crop_coords_k = None, None, None, None
            ...
            with autocast(): 
                ...
                out, contrast_losses, dn_meta, _, other_losses = self._model(data, det_targets, num_epoch, data_q, data_k, crop_coords_q, crop_coords_k)
                loss_dict, pos_indices = self._criterion(out, det_targets, seg_targets, dn_meta, num_epoch=num_epoch)
                print(f"==================cls loss={loss_dict['cls'].item()}")
                print(f"==================box loss={loss_dict['cls'].item()}")

                # 2. generate pseudo labels
                use_pseudo_label = 'pseudo_label' in self._config and self._config['pseudo_label']
                if use_pseudo_label:
                    # pseudo labels
                    pseudo_det_targets = self.get_pseudo_labels(out)
                    for i in range(len(pseudo_det_targets)):
                        print(f"==================i={i}")
                        print(f"==================labels={pseudo_det_targets[i]['labels']}")
                        print(f"==================boxes={pseudo_det_targets[i]['boxes']}")
                    ...
                    print(f"==================pseudo box loss={pseudo_loss_dict['cls'].item()}")

                # loss_dict['pixpro'] = pixpro_loss
                loss_dict['pixpro'] = other_losses['pixpro_loss']
                pixpro_losses = other_losses['pixpro_losses']

                assert len(pixpro_losses) <= 3 and len(pixpro_losses) > 0
                if len(pixpro_losses) == 1:
                    pixpro_loss4 = pixpro_losses[0].item()
                    pixpro_loss3 = 0 
                    pixpro_loss2 = 0 
                elif len(pixpro_losses) == 2: 
                    pixpro_loss4 = pixpro_losses[0].item()
                    pixpro_loss3 = pixpro_losses[1].item()
                    pixpro_loss2 = 0 
                else:
                    pixpro_loss4 = pixpro_losses[0].item()
                    pixpro_loss3 = pixpro_losses[1].item()
                    pixpro_loss2 = pixpro_losses[2].item()
            ...
            loss_pixpro2_agg += pixpro_loss2
            loss_pixpro3_agg += pixpro_loss3
            loss_pixpro4_agg += pixpro_loss4
        ...
        loss_pixpro2 = loss_pixpro2_agg / len(self._labeled_train_loader)
        loss_pixpro3 = loss_pixpro3_agg / len(self._labeled_train_loader)
        loss_pixpro4 = loss_pixpro4_agg / len(self._labeled_train_loader)
        ...
        if self._hybrid: # log many2one just if hybrid matching is activated
            self._write_to_logger(
                    ...
                    pixpro_loss2=loss_pixpro2,
                    pixpro_loss3=loss_pixpro3,
                    pixpro_loss4=loss_pixpro4,
            ...
            print(f"==================epoch={num_epoch}, total_loss={loss}, pixpro_loss={loss_pixpro}, pixpro_loss2={loss_pixpro2}, pixpro_loss3={loss_pixpro3}, pixpro_loss4={loss_pixpro4}")
