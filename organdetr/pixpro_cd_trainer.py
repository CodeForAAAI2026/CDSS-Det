...
class PixProTrainer:
    ...
    def __init__(
        self, labeled_train_loader, pixpro_train_loader, pseudo_train_loader, val_loader, model, criterion, optimizer, scheduler,
        ...
        # record last loss in case no pseudo labels
        self._last_pseudo_cls_loss = torch.tensor(0)
    ...
    def get_pseudo_labels(self, outputs):
        prob = F.softmax(outputs['pred_logits'], dim=-1)  # Shape: [2, 100, 5]
        pred_classes = prob.argmax(dim=-1)  # Shape: [2, 100]

        # Tensor shapes:
        # pred_classes: [batch_size, num_predictions] -> [2, 100]
        # Each value is the predicted class index (from 0 to 4 in this case).

        # Step 3: Get the confidence (highest probability) for each prediction
        # This tells us how "confident" the model is in its class prediction.
        confidence_values, _ = prob.max(dim=-1)  # Shape: [2, 100]

        # Tensor shapes:
        # confidence_values: [batch_size, num_predictions] -> [2, 100]
        # Each value is the maximum probability for a predicted class for each bounding box.

        # Step 4: Apply a confidence threshold to filter out low-confidence predictions
        confidence_threshold = self._config['pseudo_label_threshold']
        high_confidence_mask = confidence_values > confidence_threshold  # Shape: [2, 100]

        # Tensor shapes:
        # high_confidence_mask: [batch_size, num_predictions] -> [2, 100]
        # Each value is a boolean (True/False) indicating whether the prediction's confidence is above the threshold.

        # Step 5: Select predictions (classes and boxes) with high confidence
        # Using the mask, we filter out low-confidence predictions and retain high-confidence ones.
        high_confidence_pred_classes = [pred_classes[i][high_confidence_mask[i]] for i in range(prob.size(0))]
        high_confidence_pred_boxes = [outputs['pred_boxes'][i][high_confidence_mask[i]] for i in range(prob.size(0))]

        # Tensor shapes:
        # high_confidence_pred_classes: List of Tensors -> [Tensor1: [N1], Tensor2: [N2]]
        # high_confidence_pred_boxes: List of Tensors -> [Tensor1: [N1, 4], Tensor2: [N2, 4]]
        # Where N1 and N2 are the number of high-confidence predictions for each image in the batch.

        # Step 6: Create pseudo targets for classification and boxes
        # These pseudo targets will be used in place of ground truth for the unlabeled data.
        # pseudo_targets_class = [{"labels": high_confidence_pred_classes[i]} for i in range(prob.size(0))]
        # pseudo_targets_boxes = [{"boxes": high_confidence_pred_boxes[i]} for i in range(prob.size(0))]
        targets = [{"labels": high_confidence_pred_classes[i], "boxes": high_confidence_pred_boxes[i]} for i in range(prob.size(0))]
        return targets
    ...
    def _train_one_epoch(self, num_epoch):
        ...
        print(f"==============len(self._labeled_train_loader)={len(self._labeled_train_loader)}, len(self._pixpro_train_loader)={len(self._pixpro_train_loader)}, len(self._pseudo_train_loader)={len(self._pseudo_train_loader)}, len(self._val_loader)={len(self._val_loader)}")
        print(f"==============accu={self._config['gradient_accu']}, accu_iter={self._config['gradient_accu_iter']}")

        # progress_bar = tqdm(zip(self._labeled_train_loader, self._target_unlabeled_train_loader))
        # for idx, ((data, _, bboxes, seg_targets, path), (data_q, data_k, crop_coords_q, crop_coords_k, _)) in enumerate(progress_bar):
        progress_bar = tqdm(zip(self._labeled_train_loader, self._pixpro_train_loader, self._pseudo_train_loader))
        for idx, ((data, _, bboxes, seg_targets, _), (data_q, data_k, crop_coords_q, crop_coords_k, _), (pseudo_data, _, _)) in enumerate(progress_bar):
            # Put data to gpu
            ...
            with autocast(): 
                ...
                # 1. labeled data and pixpro
                out, contrast_losses, dn_meta, _, other_losses = self._model(data, det_targets, num_epoch, data_q, data_k, crop_coords_q, crop_coords_k, use_domain_loss)
                ...
                if use_pseudo_label:
                    ...
                    pseudo_out, _, _, _, _ = self._model(pseudo_data, None, num_epoch, data_q=None, data_k=None, crop_coords_q=None, crop_coords_k=None, domain_loss=False)
                    pseudo_det_targets = self.get_pseudo_labels(pseudo_out)
                    for i in range(len(pseudo_det_targets)):
                        print(f"==================i={i}")
                        print(f"==================labels={pseudo_det_targets[i]['labels']}")

                    # 3. use pseudo labels to train
                    # pseudo_out, _, _ , _, _ = self._model(pseudo_data, pseudo_det_targets, num_epoch, None, None, None, None)
                    # use seg_targets as dummy, only cls and box loss will be counted.
                    # ignore seg
                    self._criterion._seg_msa = False
                    pseudo_loss_dict, _ = self._criterion(pseudo_out, pseudo_det_targets, seg_targets, dn_meta, num_epoch=num_epoch)
                    # recover
                    self._criterion._seg_msa = True

                loss_dict['pixpro'] = other_losses['pixpro_loss']
                loss_dict['srcdom'] = other_losses['source_domain_loss']
                loss_dict['tgtdom'] = other_losses['target_domain_loss']
                ...
                for loss_key, loss_val in loss_dict.items():
                    if loss_key == 'srcdom':
                        loss_abs += loss_val * self._srcdom_loss_coef
                    elif loss_key == 'tgtdom':
                        loss_abs += loss_val * self._tgtdom_loss_coef 
                    else:
                        loss_abs += loss_val * self._config['loss_coefs'][loss_key.split('_')[0]]

                loss_abs += pseudo_loss_dict['cls'] * self._config['loss_coefs']['cls']
                print(f"==================pseudo cls loss={pseudo_loss_dict['cls'].item()}")

                    #print(f"=========loss_key={loss_key}, loss_val={loss_val}, loss_coefs={self._config['loss_coefs'][loss_key.split('_')[0]]}, loss={loss_val * self._config['loss_coefs'][loss_key.split('_')[0]]}, loss_total={loss_abs}")
            ...
            # source_loss_agg += source_loss_abs.item()
            loss_agg += loss_abs.item()
            # total_loss_abs = self._config['source_loss_coef'] * source_loss_abs + self._config['target_loss_coef'] * loss_abs
            ...
            if self._config['gradient_accu']:
                ...
                self._optimizer.zero_grad()
                # self._scaler.scale(0.1 * total_loss_abs).backward()
                self._scaler.scale(total_loss_abs).backward()
            ...
            loss_pixpro_agg += loss_dict['pixpro'].item()
            loss_source_domain_agg += loss_dict['srcdom'].item()
            loss_target_domain_agg += loss_dict['tgtdom'].item()
        ...
        if self._hybrid: # log many2one just if hybrid matching is activated
            ...
            print(f"==================epoch={num_epoch}, total_loss={loss}, pixpro_loss={loss_pixpro}, domain_source_loss={loss_source_domain},domain_target_loss={loss_target_domain},pseudo_cls_loss={loss_pseudo_cls}")
    ...
    def _validate(self, num_epoch):
        ...
        for data, _, bboxes, seg_targets, _ in progress_bar:
            # Put data to gpu
            data, seg_targets = data.to(device=self._device), seg_targets.to(device=self._device)
    ...
    def run(self):
        ...
        for epoch in range(self._epoch_to_start + 1, self._config['epochs'] + 1):
            if 'domain_loss_increase' in self._config and self._config['domain_loss_increase'] and epoch % self._config['domain_loss_increase_epoch'] == 0:
                new_srcdom_coef = self._srcdom_loss_coef + self._config['domain_loss_increase_value']
                new_tgtdom_coef = self._tgtdom_loss_coef + self._config['domain_loss_increase_value']
                if new_srcdom_coef < self._config['domain_loss_upper_value']:
                    self._srcdom_loss_coef = new_srcdom_coef
                if new_tgtdom_coef < self._config['domain_loss_upper_value']:
                    self._tgtdom_loss_coef = new_tgtdom_coef

                print(f"===================self._srcdom_loss_coef={self._srcdom_loss_coef}, self._tgtdom_loss_coef={self._tgtdom_loss_coef}")

            print("starting epoch ", epoch)
            self._train_one_epoch(epoch)

            '''
            if self._config['select_top_k_unlabeled'] and epoch % self._config['top_k_unlabeled_interval'] == 0:
                self._select_topk_unlabeled()
            '''
